      ...
      <div v-for="sheet in sheets" class="sheet padding-10mm" :class="{ 'sheet-shadow': !isPrinting }">
        ...
        <div class="row">
          <div v-for="col in sheet.columnsOfPaper" :style="`width: ${sheet.colWidth}%;`">
            <p :style="`margin-bottom: ${sheet.rowHeight}`" v-for="f in col" v-html="formatFormula(f)"></p>
          </div>
        </div>
      </div>
      <div class="btn" v-if="!isPrinting">
        <ElButton @click="goBack">返回</ElButton>
        <ElButton class="w-32" type="success" :loading="isExporting" @click="exportPDF">导出PDF</ElButton>
...
const sheets = computed(() => {
  return appStore.printPreviewPapers.map(p => {
    ...
    const rowHeight = solution == '0' ? '22px' : '160px'
...
const exportPDF = async () => {
  ...
  try {
    ...
    for (let i = 0; i < sheetEls.length; i++) {
      const imgData = await toJpeg(sheetEls[i], {
        quality: 0.95,
        pixelRatio: 2,
